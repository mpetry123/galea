...
class WP_Optimize_Page_Cache_Preloader extends Updraft_Task_Manager_1_2 {
	...
	private function url_is_already_cached($url, $preload_type) {
		...
		if (empty($files)) {
			...
			$gzip_is_enabled = WPO_Cache_Config::instance()->get_option('enable_gzip_compression');
			$files[] = $gzip_is_enabled ? 'index.html.gz' : 'index.html';
			if (WPO_Cache_Config::instance()->get_option('enable_mobile_caching')) {
				$files[] = $gzip_is_enabled ? 'mobile.index.html.gz' : 'mobile.index.html';
			}
